...
int ValidateRuleOrder(const std::vector<Rule>& rules) {
    ...
    CPPTEST_ASSERT(documented.size() == rules.size());
...
int main() {
    ...
    CPPTEST_ASSERT(ValidateRuleOrder(rules) == 0);
    ...
    for (const auto& ex : BuildDocExamples()) {
        ...
        CPPTEST_ASSERT(engine.Contains(ex.rule));
        CPPTEST_RUN(ValidateDocExample, engine, ex, ex.trace);
    }
