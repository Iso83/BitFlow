...
void ParseExpression(ExplorerState& state) {
    try {
        auto parsed = BitFlow::IO::Parse(state.input.c_str());
...
void DrawParsedExpressionView(const ExplorerState& state) {
    const std::string& full = state.parsedExpression.empty() ? state.input : state.parsedExpression;
...
void DrawExpressionTree(ExplorerState& state) {
    ...
    ImGui::TextDisabled("Input limit: dynamic (memory-bound)");
    if (state.expressionEditMode) {
        ImGui::InputTextMultiline("##expr_input", &state.input, ImVec2(-FLT_MIN, 140));
